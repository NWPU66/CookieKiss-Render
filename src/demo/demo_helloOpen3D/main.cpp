...
int main(int argc, char* argv[])
{
    ...
    else if (option == "editmesh")
    {
        ...
        mesh_ptr->ComputeVertexNormals();
        visualization::DrawGeometriesWithVertexSelection({mesh_ptr}, "Edit Mesh", 1600, 900);
    }
    else if (option == "spin")
    {
        auto mesh_ptr = std::make_shared<geometry::TriangleMesh>();
    ...
    else if (option == "slowspin")
    {
        ...
        mesh_ptr->ComputeVertexNormals();
        visualization::DrawGeometriesWithKeyCallbacks({mesh_ptr},
                                                      {{GLFW_KEY_SPACE,
                                                        [&](visualization::Visualizer* vis) {
                                                            vis->GetViewControl().Rotate(10, 0);
    ...
    else if (option == "image")
    {
        ...
        visualization::DrawGeometries({image_ptr}, "Image", image_ptr->width_, image_ptr->height_);
    }
    else if (option == "rgbd")
    {
        if (argc < 4)
        ...
        int  rgbd_type = utility::GetProgramOptionAsInt(argc, argv, "--rgbd_type", 0);
        ...
        std::shared_ptr<geometry::RGBDImage> (*CreateRGBDImage)(const geometry::Image&,
                                                                const geometry::Image&, bool);
        if (rgbd_type == 0)
    ...
    else if (option == "depth")
    {
        ...
        visualization::DrawGeometries(
            {pointcloud_ptr}, "geometry::PointCloud from Depth geometry::Image", 1920, 1080);
    }
    ...
    else if (option == "animation")
    {
        ...
        if (argc == 3)
        {
            visualization::DrawGeometriesWithCustomAnimation({mesh_ptr}, "Animation", 1920, 1080);
        }
        else
        {
            visualization::DrawGeometriesWithCustomAnimation({mesh_ptr}, "Animation", 1600, 900, 50,
                                                             50, argv[3]);
        }
    }
